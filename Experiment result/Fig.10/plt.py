...
if __name__ == '__main__':
	...
	plt.plot(np.array(data['loss']), np.array(data['A']) , linewidth=1, linestyle='solid',
						  markersize=8,
						  marker='o', mfc='none', mec='b', label='Base A')
	plt.plot(np.array(data['loss']), np.array(data['B']) , color='red', linewidth=1,
						   linestyle='solid', markersize=8,
						   marker='x', c='', label='Base B')
	plt.plot(np.array(data['loss']), np.array(data['Pro']), color='seagreen', linewidth=1,
			 linestyle='solid', markersize=8,
			 marker='^', c='', label='Pro')
	...
	plt.tight_layout()
	foo_fig.savefig('./loss_coverage.eps', format='eps', dpi=1000, bbox_inches='tight')
	plt.show()
